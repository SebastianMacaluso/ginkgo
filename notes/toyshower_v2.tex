...
%that is: easy to describe, has a tractable likelihood, captures the essential ingredients of the Parton shower, has python implementation with few software dependencies.



%first we study the requirements for the simplest toy generative model that would resemble a full generative model for jets. Next, we describe how to build such a model.

% =====================================================
\vspace{0.6cm}
\section{Model description}

%Maybe provide a simple narrative description of the Parton shower without physics jargon. Focus more on non-physicists, but have enough hints in the narrative description that a physicist could identify what's important. Maybe add a little dictionary about features of the model that we want to represent: running of Delta, momentum conservation, 2-d so analogue of kT clustering makes sense.
...
%2 basic features of jets:
%\begin{itemize}
%\item Momentum conservation: the total momentum of the jet (the momentum of the root of the tree) is obtained from adding the momentum of all of its constituents.
%\item Permutation invariance: the jet momentum should be invariant with respect to the order in which we cluster its constituents.
%
%\end{itemize}
%momentum conservation, i.e. the total momentum of the jet (the momentum of the root of the tree) is obtained from adding the momentum of all of its constituents.
%Jet constituents in the real world have a momentum magnitude and a direction.
%Thus for a meaningful definition of $d_{ij}$ the toy model should be at least 2D. 
...
%Basic features from a physics parton shower that the model represents:
%\begin{itemize}
%\item Momentum conservation: 
%Traditional physics jet clustering algorithms are based on an angular distance measure
%\end{itemize}
...
We define the transverse momentum as $p_{\text{T}} =|p_y|$. We show in Fig.~\ref{fig:heatclustermapTree} a heat clustermap plot of a sample jet generated with our model.

\begin{figure*}
{
  \includegraphics[width=0.85\linewidth]{plots/figTruthTruth_singlepath_jet2}
%  \includegraphics[width=0.5\linewidth]{plots/figTruthTruth_fullpath_jet2}
}
\caption{\small{Heat clustermap visualization of a Tree. Given a pair of jet constituents $\{i,j\}$ and the number of steps needed for each constituent to reach their closest common ancestor $\{s_i,s_j\}$, the heat map scale represents the maximum number of steps, i.e. $\max{\{s_i,s_j\}}$.
}}
\label{fig:heatclustermapTree}
\end{figure*}
%%-------------
