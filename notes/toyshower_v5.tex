...
\begin{algorithm}
    ...
    function NodeProcessing $(\vec{p}_{\text{p}}, \Delta_\text{p}, \Delta_\text{cut}, \lambda, \text{tree})$\\
    \Input{parent momentum $\vec{p}_{\text{p}}$, parent splitting scale $\Delta_\text{p}$, cut-off scale $\Delta_\text{cut}$, rate for the exponential distribution $\lambda$, binary tree $tree$}
    \Indp
