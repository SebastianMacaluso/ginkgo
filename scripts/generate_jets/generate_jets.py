...
def generate_jets(args):
    ...
    if args.augmented_data == 0:
        jet_list = simulator(torch.tensor(args.rate))
        logger.info(f"Starting generation of {args.num_samples} jets")
        ...
        jet_list, joint_score, joint_log_ratio, joint_log_prob = simulator.augmented_data(
            torch.tensor(args.rate), None, torch.tensor(args.rate_2), exponential=True, uniform=False
        )
        ...
        logger.info(
            f" jet_log_likelihood cross-check = {jet_log_likelihood_cross_check}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--Delta_0", type=float, default=60.0, help="Delta_0 for clustering"
    )
    parser.add_argument(
        "--jet_p",
        nargs="+",
