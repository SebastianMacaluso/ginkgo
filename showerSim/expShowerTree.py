...
class Simulator(PyroSimulator):
    ...
    def __init__(self, pt_cut=1.0, jet_pt=None, rate=1.0, Mw=None, jet_name=None):
...
def _traverse_rec(
    ...
    if Mw and id == 0:
        ...
        draw_decay = pyro.sample(
            "decay" + str(id) + str(is_left), decay_dist
        )  # We draw a number for each ball from the distribution_u
...
def _traverse(
    root, deltaMax=None, extra_info=False, sigma=None, cut_off=None, rate=None, Mw=None
):  # root should be a fj.PseudoJet
