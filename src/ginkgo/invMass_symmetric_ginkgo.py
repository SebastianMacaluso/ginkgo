...
class Simulator(PyroSimulator):
    ...
    def forward(self, inputs):
        ...
        """Sample a unit vector uniformly over a circle"""
        globals()["multiNormal_dist_circle"] = pyro.distributions.MultivariateNormal(torch.zeros(2), torch.eye(2))
        globals()["uniform"] = pyro.distributions.Uniform(0.2,np.pi/2 - 0.2)
        ...
        while len(jet_list)< self.num_samples and i<self.maxNTry:
            ...
            if self.minLeaves <= len(jet["leaves"]) < self.maxLeaves:
                ...
                logger.info(f" Total momentum from summing the leaves = {np.sum(jet['leaves'],axis=0)}")
...
def _traverse_rec(
    ...
    if delta_P > cut_off:
        ...
        # """Sample a unit vector over the 1-sphere"""
        # r_LR = pyro.sample("rLR" + str(idx) + str(is_left), multiNormal_dist_circle)
        # r_LR = r_LR.numpy()
        # r_LR = r_LR / np.linalg.norm(r_LR)
        #
        # t0 = delta_P
        # tL = t0 * draw_decay * r_LR[0]**4
        # tR = t0 * draw_decay * r_LR[1]**4
        #
        # t_tilde = t0 * draw_decay

        # theta = pyro.sample("rLR" + str(idx) + str(is_left), uniform)
        #
        # t0 = delta_P
        # tL = t0 * draw_decay**4 * (np.cos(theta))**4
        # tR = t0 * draw_decay**4 * (np.sin(theta))**4

        if tL < 0 or tR < 0:  print("tL == ", tL, " | tR= ", tR, " | tP = ", t0, " | tcut = ", cut_off)

        logger.debug(f"tL  in rest frame: {tL}")
        logger.debug(f"tR  in rest  frame: {tR}")
        logger.debug(f"----"*3)
        ...
        # logger.info(f" Off-shell subjets mass = {np.sqrt(tL), np.sqrt(tR)}")
        logger.debug(f"tL  in lab  frame: {pL_mu[0]**2  -np.linalg.norm(pL_mu[1::])**2}")
        logger.debug(f"tR  in lab  frame: {pR_mu[0] ** 2  - np.linalg.norm(pR_mu[1::]) ** 2}")
        logger.debug(f"----"*10)
        ...
        # pL_mu_rand = pL_mu
        # pR_mu_rand = pR_mu
        # draw_decay_L_rand = draw_decay
        # draw_decay_R_rand = draw_decay
