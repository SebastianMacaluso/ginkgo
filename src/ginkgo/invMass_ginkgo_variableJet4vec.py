...
class Simulator(PyroSimulator):
    def __init__(self, mean_mass=None, sigma=None, mean_p_xyz=None, covariance_vec=None, pt_cut=1.0, M_hard=None, Delta_0=None, num_samples=1, minLeaves =2 , maxLeaves=np.inf, maxNTry=20000 ):
        ...
        # self.jet_p = jet_p
        self.mean_mass = torch.tensor(mean_mass)
        self.sigma= torch.tensor(sigma)
        self.mean_p_xyz= torch.tensor(mean_p_xyz)
        self.covariance_vec=torch.tensor(covariance_vec)
    ...
    def forward(self, inputs):
        ...
        # print("Initial squared mass , mass= ", self.Delta_0, self.M_hard)
        ...
        """Sample 3 momentum from a multivariate normal"""
        globals()["P_multiNormal_dist"] = pyro.distributions.MultivariateNormal(self.mean_p_xyz, torch.diag(self.covariance_vec))
        """Sample mass from a normal distribution"""
        globals()["Mass_Normal_dist"] = pyro.distributions.Normal(self.mean_mass, self.sigma)
        ...
        while len(jet_list)< self.num_samples and i<self.maxNTry:
            ...
            jet_P = pyro.sample("p_vec" + str(i) , P_multiNormal_dist)
            jet_M = pyro.sample("jet_mass" + str(i) , Mass_Normal_dist)
            # print("Jet Mass = ", jet_M)
            # print("Jet P = ", jet_P)

            jet4vec = np.concatenate(([np.sqrt(jet_M[0] ** 2 + np.linalg.norm(jet_P) ** 2)], jet_P))
            # print("jet 4 vect = ", jet4vec)

            self.M_hard = jet_M[0]
            self.Delta_0 = self.M_hard**2
            # print("Current squared mass , mass= ", self.Delta_0, self.M_hard)
            # print("-----"*10)
            ...
            jet["mean_mass"] = self.mean_mass
            jet["sigma"] = self.sigma
            jet["mean_p_xyz"] = self.mean_p_xyz
            jet["covariance_vec"] = self.covariance_vec
